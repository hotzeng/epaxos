...
func (ep *EPaxos) startExplicitPrepare(inst common.InstRef, req common.PrepareMsg) {
    ...
    for {
        decision = psm.runPSM(inst, targetInst.Ballot.Replica, ep.peers / 2)
        if decision != reSend {
            break
        } else {
            ep.makeMulticast(req, ep.peers - 1)
...
func (psm *PrepareStateMachine) runPSM(instRef common.InstRef, leader common.ReplicaID, majority int64) (decision ExplicitPrepareDecision) {
